...
- **構成**：1ドメイン・エリア別サブディレクトリ（`/chitose` `/kitahiroshima` `/otaru` `/ebetsu`、札幌 `/sapporo` は後日noindexで追加）
...
## 使用する外部サービス
- **フォーム送信**：Web3Forms / Formspree / Static Forms 等（無料枠、`backoffice@windwoods-stayble.com` 宛、最終選定は実装時）
...
## 進行順
1. ❶ 清掃・札幌以外（このリポジトリ）：千歳 → 北広島・小樽・江別
2. ❷ 管理事業（別リポジトリ minpaku.stayble.jp）
3. ❸ 清掃・札幌（`/sapporo` を noindex で作り込み → 旧Wixより良くなったら index 切替＆旧サイトを301リダイレクト）
